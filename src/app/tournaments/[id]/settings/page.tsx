...
export default function SettingsPage({ params }: { params:{id:string} }) {
  ...
  async function save(e:React.FormEvent){
    ...
    if(res.ok){toast.success('Saved');setTName(name)}else toast.error('Failed')
    setSaving(false)
  }
    ...
    <div className="max-w-xl">
      ...
      <form onSubmit={save} className="space-y-5">
        ...
        <div className="card p-5">
          ...
          <div><label className="label">Tournament Name</label><input className="input" value={name} onChange={e=>setName(e.target.value)} required/></div>
        </div>
        ...
        <div className="card p-5">
          ...
          <div className="space-y-1.5 mb-4">
            {DEFAULT_DIVISIONS.map((defaultDiv, i) => {
              const isChecked = divisions.some(d => d === defaultDiv || divisions.indexOf(d) === i)
              const currentName = divisions.find(d => d === defaultDiv) || defaultDiv
