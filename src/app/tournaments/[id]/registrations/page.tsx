                ...
                <div key={reg.id} className="bg-white border border-slate-200 rounded-xl overflow-hidden">
                  ...
                  {expanded === reg.id && (
                    <div className="border-t border-slate-100 bg-slate-50 px-3 sm:px-5 py-4 space-y-4">
                        ...
                        <table className="w-full min-w-[600px] text-sm border-collapse">
                          <thead>
                            <tr className="bg-slate-100 text-slate-600 text-xs uppercase">
                              {['Club','Team','Division','Waivers','Coach','Phone','Email'].map(h => (
                                <th key={h} className={`px-3 py-2 ${h === 'Waivers' ? 'text-center' : 'text-left'}`}>{h}</th>
                              ))}
                            </tr>
                          </thead>
                          <tbody>
                      ...
                      {reg.teams.length > 0 && (() => {
                        const total = reg.teams.reduce((sum, t) => sum + (t.waiverCount ?? 0), 0) + (reg.waiverUnassigned ?? 0)
