...
export async function POST(req: NextRequest) {
  ...
  const letter = await commLetterFor((org as { id?: string } | null)?.id ?? null, kind)
  ...
  const cta = COMM_KINDS[kind].cta
  const ctaUrl = cta === 'waiver' ? waiverLink : cta === 'schedule' ? scheduleLink : ''
  const now = new Date().toISOString()

  const results: { regId: string; status: 'sent' | 'no_email' | 'failed' }[] = []
  ...
  ${ctaUrl ? `<p style="text-align:center;margin:24px 0"><a href="${ctaUrl}" style="background:#0d9488;color:#ffffff;text-decoration:none;padding:12px 28px;border-radius:8px;font-weight:bold;display:inline-block">${COMM_KINDS[kind].ctaLabel}</a></p>
  ...
  <p style="font-size:13px;color:#64748b">Questions? Just reply to this email.</p>
    ...
    const text = `${bodyText}${ctaUrl ? `\n\n${COMM_KINDS[kind].ctaLabel}: ${ctaUrl}` : ''}`
