...
function PostDrawer({ post, metrics, siblings, queue, mode, setMode, canApprove, onClose, onApprove, onPublishNow, onPatch, onDelete }: { post: Post; metrics?: PostMetrics; siblings: Post[]; queue: QueueInfo; mode: 'preview' | 'edit'; setMode: (m: 'preview' | 'edit') => void; canApprove: boolean; onClose: () => void; onApprove: (p: Post) => void; onPublishNow: (p: Post) => Promise<void>; onPatch: (id: string, body: any, msg?: string) => Promise<boolean>; onDelete: (id: string, msg: string) => void }) {
  ...
  const statusLine = post.status === 'draft' ? <div className="rounded-xl bg-amber-50 text-amber-800 text-xs font-bold px-3 py-2.5">Needs approval · scheduled for {fmtLong(when)}</div>
    ...
    : <div className="rounded-xl bg-emerald-50 text-emerald-800 text-xs font-bold px-3 py-2.5">{post.lastError || (post.mediaType === 'video' && a.platform === 'instagram' ? 'Publishing — Instagram is processing the video…' : 'Publishing right now…')}</div>
  return (
    <>
      ...
      <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-3.5">
        ...
        {post.status === 'publishing' && <div className="text-xs text-slate-500">{a.platform === 'instagram' && post.mediaType === 'video' ? 'Instagram usually finishes a video in 1–2 minutes and this updates on its own. ' : ''}If it's been more than a few minutes, <b>Check now</b> looks on the account and settles it — Published if it's live, safe-to-retry if it never went out.</div>}
      </div>
      <div className="px-4 py-3 border-t border-slate-200 flex flex-wrap gap-2">
        ...
        {post.status === 'publishing' && <button onClick={() => onPatch(post.id, { action: 'check' }).then(ok => ok && toast('Checked the account — status updated'))} className="rounded-xl bg-white border border-slate-300 text-slate-700 hover:bg-slate-50 font-bold text-sm px-4 py-2.5 inline-flex items-center gap-1.5"><RotateCcw size={14} /> Check now</button>}
